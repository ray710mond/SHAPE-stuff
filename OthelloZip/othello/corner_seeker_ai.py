...
def minimax_min_node(board, color, depth):
    ...
    possible_moves = implement_bias(board, color, possible_moves)
...
def select_move_minimax(board, color, depth):
    ...
    time_duration = 9.99999999999999999999999999999999999999999999
    discovered = []
    possible_moves = get_possible_moves(board, color)
    possible_moves = implement_bias(board, color, possible_moves)
    while time.time() < time_start + time_duration and len(possible_moves) > len(discovered):
        opp_color = 1 if color == 2 else 2
        max = -math.inf
        best_move = (0,0)
        for possible_move in possible_moves:
            discovered.append(possible_move)
            new_board = play_move(board, color, possible_move[0], possible_move[1])
        ...
        logging.info("in " + str(round(time.time() - time_start, 3)) + " secs")
        return best_move


def implement_bias(board, color, possible_moves):
...
def select_move_alphabeta(board, color, alpha, beta):
    ...
    while time.time() < time_start + time_duration and len(possible_moves) > len(discovered): 
        ...
        for possible_move in possible_moves:
            ...
            discovered.append(possible_move)
        logging.basicConfig(level = logging.INFO)
        logging.info("Corner Seeker played " + str(best_move) + " in " + str(round(time.time() - time_start, 3)) + " secs")
    return best_move
...
def run_ai():
    ...
    def saveData():
        ...
        with open("ai_data.txt", "r") as f:
            ...
            for line in f:
                lines.append(line)
            test1.append(lines[0])
            test2.append(lines[1])
            dark, light = get_score(board)
